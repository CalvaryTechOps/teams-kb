...
export async function AppSidebar({
  ...
  return (
    <aside className="flex h-full w-[268px] shrink-0 flex-col overflow-y-auto bg-sidebar pb-5">
      ...
      <form action="/search" className="px-4 pb-4 pt-1">
        <div className="group flex items-center gap-2 rounded-lg bg-sidebar-fg/10 px-2.5 focus-within:shadow-focus">
          <SearchIcon size={14} className="shrink-0 text-sidebar-fg-muted" />
          <input
            id={SIDEBAR_SEARCH_ID}
            type="search"
            name="q"
            placeholder="Search articles"
            className="h-9 w-full bg-transparent text-[13px] text-sidebar-fg placeholder-sidebar-fg-muted focus:outline-none"
          />
          {/* Shortcut hint; hides while typing. Keyboard-only, so md+ only. */}
          <kbd
            aria-hidden
            className="hidden rounded border border-sidebar-fg/15 px-1.5 text-[11px] leading-5 text-sidebar-fg-muted md:inline md:group-focus-within:hidden"
          >
            /
          </kbd>
        </div>
      ...
      <div className="mt-auto flex items-center gap-2.5 border-t border-sidebar-fg/10 px-5 pt-4">
        ...
        <div className="min-w-0 flex-1 leading-tight">
          <div className="truncate text-[13px] font-medium text-sidebar-fg">
            {userName}
          </div>
          <div className="text-[11px] text-sidebar-fg-subtle">
